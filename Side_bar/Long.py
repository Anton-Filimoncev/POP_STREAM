...
def long():
    ...
    with col12:
        ...
        long_strike = st.number_input('Strike', step=0.01, format="%.2f", min_value=0., max_value=50000., value=170.)
        long_price = st.number_input('Price', step=0.01, format="%.2f", min_value=0., max_value=50000.,
                                            value=9.16)

    with col13:
        sigma = st.number_input('Volatility Mean', step=0.01, format="%.2f", min_value=0., max_value=50000., value=36.)

        days_to_expiration = st.number_input('Days to EXP', step=1,  min_value=0, max_value=50000, value=256)
